...
const VuePropTypes = {
  ...
  custom(validatorFn: any, warnMsg = 'custom validation failed') {
    ...
    return toType(validatorFn.name || '<<anonymous function>>', {
      validator(...args: any[]) {
        ...
        if (!valid) warn(`${(this as any)._vueTypes_name} - ${warnMsg}`)
        return valid
      },
    })
  },
  ...
  arrayOf(type: { type: any } | null | undefined) {
  ...
  objectOf(type: { type: any } | null | undefined) {
      ...
      validator(value: { [x: string]: any }) {
        ...
        return valueKeys.every((key) => {
          if (keys.indexOf(key) === -1) {
            if ((this as any)._vueTypes_isLoose === true) return true
